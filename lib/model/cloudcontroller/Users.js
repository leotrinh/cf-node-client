...
class Users extends CloudControllerBase {
    ...
    getUsers(filter) {
        ...
        if (this.isUsingV3()) {
            return this.REST.requestV3("GET", `${this.API_URL}/v3/users`, token);
        }
        const options = {
            method: "GET",
    ...
    remove(guid) {
        const token = this.getAuthorizationHeader();
        if (this.isUsingV3()) {
            return this.REST.requestV3("DELETE", `${this.API_URL}/v3/users/${guid}`, token, null, this.HttpStatus.NO_CONTENT);
        }
        const options = {
            method: "DELETE",
            url: `${this.API_URL}/v2/users/${guid}`,
