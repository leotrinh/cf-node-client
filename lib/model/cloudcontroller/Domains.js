...
class Domains extends CloudControllerBase {
    ...
    add(domainOptions) {
        ...
        if (this.isUsingV3()) {
            ...
            if (domainOptions.organization_guid) {
                v3Body.relationships = {
                    organization: { data: { guid: domainOptions.organization_guid } }
                };
            }
    ...
    remove(guid) {
        const token = this.getAuthorizationHeader();
        if (this.isUsingV3()) {
            return this.REST.requestV3("DELETE", `${this.API_URL}/v3/domains/${guid}`, token, null, this.HttpStatus.ACCEPTED);
        }
        const options = {
            method: "DELETE",
            url: `${this.API_URL}/v2/domains/${guid}`,
