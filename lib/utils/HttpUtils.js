...
class HttpUtils {
     ...
     * @param  {Number} httpStatusAssert [expected HTTP status code (200, 201, 204, etc.)]
    ...
    async request(options, httpStatusAssert, jsonOutput) {
        ...
        if (response.status !== httpStatusAssert) {
            ...
            try {
                ...
                const err = new Error(errJson.description || errJson.message || text);
     ...
     * @param  {Object} options          [upload options: accessToken, data, query, multipart]
